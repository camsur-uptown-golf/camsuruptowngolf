      ...
      <main className="bg-[#f7f5ee] text-[#14271d]">
        ...
        <section className="overflow-hidden bg-[#f7f5ee] py-14 sm:py-18 lg:py-20">
          <div className="mx-auto grid max-w-7xl gap-10 px-6 md:grid-cols-[250px_minmax(0,1fr)] lg:gap-16 lg:px-8">
            <aside className="border-[#1f3f2e]/55 md:border-r md:pr-10">
              <h2 className="text-2xl font-semibold tracking-[-0.025em]">Itineraries</h2>
              <div className="mt-7 h-px w-36 bg-[#1f3f2e]/70" />
              <nav className="mt-7 space-y-4 font-navigation text-[10px] font-bold uppercase leading-[1.35] tracking-[0.08em] text-[#455249]">
                {PACKAGE_LINKS.map(([slug, label]) => (
                  <Link key={slug} href={`/packages/${slug}`} className={`block transition hover:text-[#265136] ${active === slug ? "text-[#265136]" : ""}`}>
                    {active === slug ? "— " : ""}{label}
                  </Link>
                ))}
              </nav>
                ...
                {days.map((day, index) => (
                  <details key={day.label} className="group border-b border-[#1f3f2e]/25">
                    ...
                    <div className="pb-8 sm:pr-14">
                      ...
                      <div className="mt-6 space-y-4">{day.moments.map(([label, copy]) => <div key={label} className="grid gap-1 sm:grid-cols-[70px_1fr] sm:items-baseline sm:gap-4"><p className="font-navigation text-[9px] font-bold uppercase tracking-[0.15em] text-[#265136]">{label}</p><p className="text-sm leading-7 text-[#59665e]">{copy}</p></div>)}</div>
                    </div>
                  </details>
                ))}
              </div>
