...
import { findImagesMissingAlt } from '../utils/helpers';

/**
 * QA Task 10 — Accessibility & Keyboard Navigation (TC-279 … TC-296) = 18 cases.
...
test.describe('Task 10 · Accessibility & Keyboard', () => {
  test('01 — Keyboard-only navigation reaches interactive elements', async ({ page }) => {
    await page.goto('/');
  ...
  test('02 — Visible focus indicator on interactive elements', async ({ page }) => {
    await page.goto('/');
  ...
  test('03 — Skip-to-content link appears on first Tab', async ({ page }) => {
    await page.goto('/');
    await page.keyboard.press('Tab');
  ...
  test('04 — Mega-menu is keyboard operable', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('button', { name: /^GOLF$/i }).focus();
    await page.keyboard.press('Enter');
    await expect(page.getByRole('link', { name: 'Hole No. 1' })).toBeVisible();
  });

  test('05 — Mobile drawer manages focus', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 375, height: 812 } });
    const page = await ctx.newPage();
    await page.goto('/');
    const burger = page.getByRole('button', { name: /menu|navigation/i }).or(page.locator('button[aria-label*="menu" i]')).first();
    if ((await burger.count()) === 0) { await ctx.close(); test.skip(true, 'No hamburger — confirm selector.'); }
    await burger.click();
    await expect(page.locator('nav, [role="dialog"]').first()).toBeVisible();
    await ctx.close();
  });

  test('06 — Form fields have associated labels', async ({ page }) => {
    await page.goto('/contact');
    const violations = (await axeViolations(page)).filter((v) => v.id === 'label');
    expect(violations, JSON.stringify(violations, null, 2)).toEqual([]);
  });

  test('07 — Form errors are announced (aria)', async ({ page }) => {
    await page.goto('/contact');
  ...
  test('08 — Colour contrast (text) meets AA', async ({ page }) => {
    await page.goto('/');
  ...
  test('09 — Colour contrast (buttons/links) meets AA', async ({ page }) => {
    await page.goto('/contact');
    const violations = (await axeViolations(page, ['wcag2aa'])).filter((v) => v.id === 'color-contrast');
    expect(violations).toEqual([]);
  });

  test('10 — Heading hierarchy: single H1 per page', async ({ page }) => {
  ...
  test('11 — Meaningful images have alt text', async ({ page }) => {
    await page.goto('/');
    expect(await findImagesMissingAlt(page)).toEqual([]);
  });

  test('12 — Landmarks / ARIA roles present', async ({ page }) => {
    await page.goto('/');
    const violations = (await axeViolations(page)).filter((v) => v.id.includes('landmark') || v.id.includes('region'));
    expect(violations).toEqual([]);
  });

  test('13 — Lightbox / dialog traps focus and closes', async ({ page }) => {
  ...
  test('14 — No keyboard trap (can Tab through and back)', async ({ page }) => {
    await page.goto('/');
  ...
  test('15 — Reduced-motion preference respected', async ({ browser }) => {
  ...
  test('16 — Content reflows at 200% zoom', async ({ page }) => {
    await page.goto('/');
  ...
  test('17 — Link purpose is clear (no bare "click here")', async ({ page }) => {
    await page.goto('/');
  ...
  test('18 — Touch targets are at least ~44px (mobile)', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 375, height: 812 } });
    const page = await ctx.newPage();
    await page.goto('/');
